...
from . import db
from .tools import LOCK_TIMEOUT, _file_lock
...
def review_suggestion(patch_id: int, action: str) -> Tuple[bool, str]:
    ...
    # ---- 应用:先加文件锁,再校验落盘(锁覆盖"读取→校验→写入"全程) ----
    ...
    # 文件锁:与 Agent 验证补丁(run_pytest)共用同一把锁——
    # 防止"审核落盘"与"临时验证替换"并发交错,互相覆盖/还原对方的写入
    lock = _file_lock(path)
    try:
        lock.acquire(timeout=LOCK_TIMEOUT)
    except Timeout:
        return False, f"目标文件 {row['file_path']} 正被其他修复任务锁定,请稍后重试"
    try:
